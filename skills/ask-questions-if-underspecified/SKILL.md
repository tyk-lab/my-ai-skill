...
If the current runtime supports interactive tools, pick any available tool suitable for collecting user input — common examples include `AskUserQuestion`, `ask_user`, `request_user_input`, `question`, and similar. Choose whichever one works in the current environment; there is no required order.

If none of those tools are available or supported, ask the same questions directly in chat (plain text).
...
Tool requirement:
- If clarification is required, use the question tool (see "Question Tool" above) instead of plain chat questions.
